...
/**
 * The single source of truth for brand/UI glyph paths (Heroicons outline).
 *
 * This lives here rather than inside the Blade component because FaviconController
 * needs the same paths. It previously kept its own partial copy, and because that
 * copy had no entry for this product's glyph the favicon silently fell back to the
 * scaffold's shield: the tab icon was another product's mark. One registry, no drift.
 */
class Icons
{
    public const PATHS = [
        ...
        'restore' => '<path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182m0-4.991v4.99" />',
        ...
        // The SpamMGR brand glyph: mail passing through a filter.
        ...
        'key' => '<path stroke-linecap="round" stroke-linejoin="round" d="M15.75 5.25a3 3 0 0 1 3 3m3 0a6 6 0 0 1-7.029 5.912c-.563-.097-1.159.026-1.563.43L10.5 17.25H9v1.5H7.5v1.5H6v1.5H2.25v-1.5l4.108-4.108a1.5 1.5 0 0 1 .43-1.563A6 6 0 1 1 21.75 8.25Z" />',
    ...
    public static function path(string $name, ?string $fallback = null): ?string
    {
        return self::PATHS[$name] ?? ($fallback !== null ? (self::PATHS[$fallback] ?? null) : null);
    }
