...
class AuthController extends Controller
{
    ...
    /**
     * One-click sign-in for a test account, restricted to configured IPs.
     *
     * The gate is re-checked here rather than trusting that the button was rendered:
     * hiding a control is presentation, not access control. See App\Services\DevLoginGate
     * for why the check cannot use $request->ip() on this host.
     */
    public function devLogin(Request $request, \App\Services\DevLoginGate $gate)
    {
        abort_unless($gate->allows($request), 404);
        ...
        Auth::login($user);
        $request->session()->regenerate();
        \App\Models\AuditLog::record('login', 'Signed in via the IP-restricted test button');
