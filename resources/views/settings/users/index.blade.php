<x-layouts.app title="Users">
    ...
    <x-card flush>
        <x-table flush>
            <thead>
                <tr><th>Name</th><th>Email</th><th>Role</th><th>Customer</th><th class="text-right">Actions</th></tr>
            </thead>
            <tbody>
                @foreach ($users as $u)
                    <tr>
                        ...
                        <td class="text-slate-500">{{ $u->customer?->name ?? "None" }}</td>
                        <td class="text-right">
                            <div class="inline-flex items-center gap-2">
                                ...
                                @if ($u->id !== auth()->id())
                                    <x-delete-button :name="'del-user-' . $u->id" :action="route('settings.users.destroy', $u)"
                                        title="Delete User?" message="Their domains and mail are kept; only the login is removed." />
                                @endif
                            </div>
                        </td>
                    </tr>
